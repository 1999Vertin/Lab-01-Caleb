...
There are 1846 rows and 3 variables. The variables are **dataset**, **x**, **y**
...
```
```{marginfigure}
Start with `dstar_data` and calculate a summary statistic that we will call `r` as the `cor`relation between `x` and `y`.
```
```{r}
star_data %>%
  summarize(star = cor(x, y))
```
